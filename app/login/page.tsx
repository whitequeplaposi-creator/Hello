...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { setError(result.error || t('wrongCredentials')) }
  }
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-6">
        <div className="w-full max-w-2xl">
          <div className="mb-4 text-center">
            <h1 className="text-xl font-bold text-gray-900 mb-1">{t('welcomeBack')}</h1>
            <p className="text-gray-500 text-xs">{t('loginSubtitle')}</p>
          </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-2.5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPassword ? 'text' : 'password'} id="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-1.5 text-sm bg-gray-50 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-black focus:border-transparent transition-all pr-10 text-gray-900" disabled={isLoading} />
                ...
                <button type="submit" disabled={isLoading} className="w-full px-3 py-1.5 text-sm bg-gray-900 text-white font-medium rounded-md hover:bg-gray-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center">
                  {isLoading ? (
                    <><svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{t('loggingIn')}</>
                  ) : t('login')}
                </button>
              </form>
            </div>
          ...
          <div className="mt-4 text-center">
            <p className="text-gray-600 text-xs">
              {t('noAccount')}{' '}
              <Link href="/registrera" className="text-black font-medium hover:underline">{t('createAccountHere')}</Link>
            </p>
          </div>
        </div>
      </main>
